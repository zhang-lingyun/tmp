...
class ConstantInput(nn.Module):
    def __init__(self, channel, height=4,width=3):
        super().__init__()

        self.input = nn.Parameter(torch.randn(1, channel, height,width))
...
class Decoder(nn.Module):
    def __init__(
        ...
        size,
        style_dim,
        latent_spatial_size1,
        latent_spatial_size2,
        channel_multiplier,
        blur_kernel,
        normalize_mode,
        lr_mul,
        ...
        self.input = ConstantInput(
            channels[latent_spatial_size1], height=latent_spatial_size1,width=latent_spatial_size2
        )

        if small_generator:
            stylecode_dim = style_dim
        else:
            stylecode_dim = channels[latent_spatial_size1]

        self.conv1 = StyledConv(
            channels[latent_spatial_size1],
            channels[latent_spatial_size1],
            3,
            stylecode_dim,
        ...
        in_channel = channels[latent_spatial_size1]

        self.start_index = int(math.log(latent_spatial_size1, 2)) + 1  # if 4x4 -> 3
        ...
        self.latent_spatial_size1 = latent_spatial_size1
        self.latent_spatial_size2 = latent_spatial_size2
    ...
    def __init__(
        ...
        for i in range(mapping_layer_num):
            if i != (mapping_layer_num - 1):
                ...
                in_channel = style_dim
                out_channel = style_dim * latent_spatial_size1 * latent_spatial_size2
        ...
        if calculate_mean_stylemap:  # calculate mean_latent
            ...
            if input_is_stylecode:
                ...
                stylecode = stylecode.reshape(
                    N, -1, self.latent_spatial_size1, self.latent_spatial_size2
                )

            image = self.decoder(stylecode, mix_space=mix_space, mask=mask)
            cloth,person,pose,mask = image[:,:3,:,:],image[:,3:6,:,:],image[:,6:9,:,:],image[:,9:,:,:]

            if return_stylecode == True:
                return cloth,person,pose,mask, stylecode
            else:
                return cloth,person,pose,mask, None


class Discriminator(nn.Module):
    def __init__(self, size, channel_multiplier=2, blur_kernel=[1, 3, 3, 1],input_channel=3):
        super().__init__()
        ...
        self.final_linear = nn.Sequential(
            EqualLinear(channels[4] * 4 * 3, channels[4], activation="fused_lrelu"),
...
class Encoder(nn.Module):
    def __init__(
        ...
        self.from_rgb = ConvLayer(input_channels, channels[size], 1)
        self.convs = nn.ModuleList()
